...
Raw parameter estimates from complex models, particularly those involving non-linear terms or interactions, often prove challenging to interpret and may lack real-world context. This difficulty is especially pronounced for non-statisticians, who are often the very individuals interested in these effects and tasked with translating results into practical solutions.\
The term "marginal effects" carries slightly different meanings across different fields and contexts. For an in-depth exploration of this terminology, I recommend to read [Andrew Heiss's blog post](https://www.andrewheiss.com/blog/2022/05/20/marginalia/). For the following, I'll adhere to the following general concept of marginal effects: Marginal effects illustrate how a one-unit change in the independent variable influences the dependent variable, while holding other factors or covariates constant. This concept manifests differently across model types:
...
When dealing with continuous variables for both predictors and outcomes, marginal effects become a powerful tool for interpretation of non-linear models like quadratic relationships, where raw regression estimates prove challenging to interpret. Let's explore this concept using the `PimaIndiansDiabetes` dataset, where we'll examine the relationship between body mass and age. While this model is primarily for illustrative purposes and doesn't seek deeper interpretation, it serves as a nice example of marginal effects in action.

<br>

### Linear vs. Quadratic: Which Fits Better?

First, let's compare linear and quadratic models to see which better describes our data:
...
Now, let's visualize these slopes on our plot:
...
This visualization helps us understand how the relationship between age and body mass changes over time. We can see that the slope is positive at age 30 but negative at age 60, indicating a shift in the relationship.
...
This shift from a positive to a negative relationship as age increases illustrates the non-linear nature of the association between age and body mass in our dataset.

<br><br>

### Beyond MER: Exploring MEM and AME

We've just taken a deep dive into the concept of *Marginal Effects at Representative Values (MER)*. But there's more! Two other concepts can shed light on our data (depending on context and data, one or the other make more sense): MEM and AME. Let's break these three down in more detail:

::: columns
::: {.column width="55%"}
<br><br> **Marginal Effect at Representative Values (MER)**:

This represents the dy/dx at very specific, representative values of x (age in the example above).
...
Logistic regression is a powerful and often used tool, but its raw parameters can be challenging to interpret. Many practitioners struggle with the concept of log odds ratios and often mistakenly interpret odds as probabilities. This is where marginal effects come to the rescue, offering a more intuitive way to understand and communicate the results of logistic regression models.

In logistic regression, the raw parameters represent log odds ratios, expressed as $log(\frac{p}{1-p})$. When we exponentiate these coefficients, we get odds ratios $\frac{p}{1-p}$. But what do these really mean in practical terms? The interpretation can be tricky, and it's easy to conflate odds with probabilities. While it would be convenient if they were the same, they're not – and this is where marginal effects shine.
...
Furthermore, to get a population-level perspective, we can average these individual effects using `avg_comparisons()`
...
If we don't specify the `variables` argument, the function calculates marginal effects at the means of both predictors:
